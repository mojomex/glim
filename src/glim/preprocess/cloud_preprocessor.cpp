...
PreprocessedFrame::Ptr CloudPreprocessor::preprocess_impl(const RawPoints::ConstPtr& raw_points) {
  ...
  for (int i = 0; i < frame->size(); i++) {
    ...
    const bool is_below_100ms = frame->times[i] < 100e-3;
    if (squared_dist > squared_distance_near_thresh && squared_dist < squared_distance_far_thresh && is_finite && is_below_100ms) {
      indices.push_back(i);
    }
  }
